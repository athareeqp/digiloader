...
export default function Posts() {
  ...
  const searchItems = (searchValue) => {
    ...
    if (searchInput !== "") {
      const filteredData = DigimonsData.filter((Digimon) => {
        return Object.values(Digimon).join("").toLowerCase().includes(searchInput.toLowerCase());
      });
  ...
  return (
    <div>
      <Head>
        <title>DigiLoader</title>
      </Head>
      <div className="search-bar">
        <form>
          <input className="form-control mr-sm-2" type="text" placeholder="Search Digimon or Level" aria-label="Search" onChange={(e) => searchItems(e.target.value)} />
        </form>
      </div>
              ...
              <div className="all-container">
                {searchInput.length > 1
                  ? filteredResults.map((Digimon, index) => {
                      return <DigimonThumb key={index} img={Digimon.img} name={Digimon.name} level={Digimon.level} />;
                    })
